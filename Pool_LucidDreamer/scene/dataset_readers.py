...
def readCircleCamInfo(path,opt):
    ...
    if not os.path.exists(ply_path):
        ...
        elif opt.init_shape == 'our' or opt.init_shape == 'our_xyz':
            # init_prompt = 'data/pointe_our.pth'
            xyz = torch.load(opt.init_prompt)
            xyz = xyz.cpu().numpy()
            num_pts = xyz.shape[0]
        else:
        ...
        if opt.init_shape == "our":
            if "shape_e" in opt.init_prompt or "point_e" in opt.init_prompt:
                rgb = torch.load(opt.init_prompt.replace('xyz','rgb'))
                rgb = rgb.cpu().numpy()
                pcd = BasicPointCloud(points=xyz, colors=rgb, normals=np.zeros((num_pts, 3)))
                storePly(ply_path, xyz, rgb * 255)
            else:
                shs = torch.load(opt.init_prompt.replace('xyz','color')).squeeze(1)
                shs = shs.cpu().numpy()
