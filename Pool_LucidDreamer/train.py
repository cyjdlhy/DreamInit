...
def guidance_setup(guidance_opt):
    if guidance_opt.guidance=="SD":
        ...
        # from guidance.sd_utils_IF import IF
    ...
    for iteration in range(first_iter, opt.iterations + 1):        
        ...
        with torch.no_grad():
            ...
            if iteration < opt.densify_until_iter:
                ...
                if iteration > opt.opacity_from_iter and iteration % opt.opacity_reset_interval == 0: #or (dataset._white_background and iteration == opt.densify_from_iter)
...
def prepare_output_and_logger(args):    
    if not args._model_path:
        ...
        args._model_path = os.path.join("./output_rebuttal/", args.workspace)
...
def video_inference(iteration, scene : Scene, renderFunc, renderArgs):
    ...
    config = ({'name': 'test', 'cameras' : scene.getCircleVideoCameras(batch_size=500)})
    if config['cameras'] and len(config['cameras']) > 0:
        img_frames = []
        depth_frames = []
...
if __name__ == "__main__":
    ...
    # others
    # parser.add_argument("--opacity_reset_interval", type=int, default=500)
    # parser.add_argument("--workspace", type=str, default='')
    # parser.add_argument("--text", type=str, default='')
    # parser.add_argument("--init_shape", type=str, default='')
    # parser.add_argument("--init_prompt", type=float, default=1.0) # 这里是放xyz和rgb
    ...
    if args.opt is not None:
        ...
        op.opacity_reset_interval = args.opacity_reset_interval
        op.opacity_from_iter = args.opacity_from_iter
