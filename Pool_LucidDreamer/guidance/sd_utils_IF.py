...
from diffusers import StableDiffusionPipeline, DiffusionPipeline, DDPMScheduler, DDIMScheduler, EulerDiscreteScheduler, \
                      ...
                      DDIMInverseScheduler, UNet2DConditionModel, IFPipeline
...
class IF(nn.Module):
    ...
    def train_step(self, text_embeddings, pred_rgb, pred_depth=None, pred_alpha=None,
        ...
        if as_latent:      
            # latents,_ = self.encode_imgs(pred_depth.repeat(1,3,1,1).to(self.precision_t))
            # 一会最好看一下这个depth的范围
            latents = F.interpolate(pred_depth.repeat(1,3,1,1), (64, 64), mode='bilinear', align_corners=False) * 2 - 1
        else:
            # latents,_ = self.encode_imgs(pred_rgb.to(self.precision_t)) # 原来pred_rgb是 0-1
            latents = F.interpolate(pred_rgb, (64, 64), mode='bilinear', align_corners=False) * 2 - 1
        # timestep ~ U(0.02, 0.98) to avoid very high/low noise level

        if self.noise_temp is None:
            self.noise_temp = torch.randn((latents.shape[0], 3, resolution[0] // 8, resolution[1] // 8, ), dtype=latents.dtype, device=latents.device, generator=self.noise_gen) + 0.1 * torch.randn((1, 3, 1, 1), device=latents.device).repeat(latents.shape[0], 1, 1, 1)
        
        if guidance_opt.fix_noise:
            noise = self.noise_temp
        else:
            noise = torch.randn((latents.shape[0], 3, resolution[0] // 8, resolution[1] // 8, ), dtype=latents.dtype, device=latents.device, generator=self.noise_gen) + 0.1 * torch.randn((1, 3, 1, 1), device=latents.device).repeat(latents.shape[0], 1, 1, 1)
        ...
        with torch.no_grad():
            latent_model_input = latents_noisy[None, :, ...].repeat(2, 1, 1, 1, 1).reshape(-1, 3, resolution[0] // 8, resolution[1] // 8, )
            tt = t.reshape(1, 1).repeat(latent_model_input.shape[0], 1).reshape(-1)
            ...
            unet_output = unet_output.reshape(2, -1, 3, resolution[0] // 8, resolution[1] // 8, )
            noise_pred_uncond, noise_pred_text = unet_output[:1].reshape(-1, 3, resolution[0] // 8, resolution[1] // 8, ), unet_output[1:].reshape(-1, 3, resolution[0] // 8, resolution[1] // 8, )
